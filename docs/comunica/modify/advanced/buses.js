 ...
 */
const rawRepoFiles =
  "https://raw.githubusercontent.com/comunica/comunica/refs/heads/master/";
  ...
  return (
    <>
      ...
      <p>
        <em>
          Package:{" "}
          <a href={`${treeRepoFiles}packages/bus-${busName}`}>
            <code>@comunica/bus-{busName}</code>
          </a>
        </em>
      </p>
      <p>
      ...
      <div className="container-page">
        ...
        <main>
          <ul className="breadcrumbs">
            ...
            <li>
              <a href="/docs/modify/">Modify Comunica</a>
            </li>
            <li>
              <a href="/docs/modify/advanced/">Advanced modification</a>
            </li>
          ...
          <p>
            This page gives an{" "}
            <strong>
              overview of all <em>buses</em> and <em>actors</em>
            </strong>{" "}
            that are used in the default Comunica engines, such as{" "}
            <a href="https://github.com/comunica/comunica/tree/master/engines/query-sparql">
              Comunica SPARQL
            </a>{" "}
            and{" "}
            <a href="https://github.com/comunica/comunica/tree/master/engines/query-sparql-file">
              Comunica SPARQL File
            </a>. Other configurations such as{" "}
            <a href="https://github.com/comunica/comunica-query-sparql-hdt">
              Comunica SPARQL HDT
            </a>{" "}
            contain additional actors and buses.
          </p>
          <p>
            This builds upon the{" "}
            <a href="/docs/modify/advanced/architecture_core/">
              core architecture
            </a>{" "}
            of <em>actors</em>, <em>mediators</em>, and{" "}
            <em>buses</em>. An overview of how these buses and actors are
            connected can be found in the{" "}
            <a href="/docs/modify/advanced/architecture_sparql/">
              SPARQL architecture
            </a>.
          </p>
          {busInfo}
...
async function generateActorInfo(actor) {
  ...
  const actorReadme = await fetchFile(
    `${rawRepoFiles}packages/${actorName}/README.md`,
  );
  ...
  if (!descriptionMatch) {
    throw new Error(
      `No description found for actor ${actorName} parsed:\n${actorReadme}`,
    );
  }
...
async function generateBusInfo(busName, actors) {
  const busPackage = await fetchJson(
    `${rawRepoFiles}packages/bus-${busName}/package.json`,
  );
...
export async function getStaticProps({ ...ctx }) {
  ...
  const defaultConfig = await fetchJson(
    `${rawRepoFiles}engines/query-sparql/package.json`,
  );
  const actors = Object.keys(defaultConfig.dependencies).filter((dep) =>
    dep.startsWith("@comunica/actor-")
  );

  const actorInfo = await Promise.all(
    actors.map((actor) => generateActorInfo(actor)),
  );
  ...
  const bussesInfo = await Promise.all(
    Object.keys(busActors).map((busName) =>
      generateBusInfo(busName, busActors[busName])
    ),
  );
