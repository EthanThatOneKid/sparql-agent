...
[the guide on querying in a JavaScript app](/docs/query/getting_started/query_app/),
...
We make use of the
[`Store` from `N3.js`](https://github.com/rdfjs/N3.js#storing) for these
examples.

## 2. Executing INSERT DATA queries

Once you engine has been created, you can use it to execute any SPARQL Update
query, such as a `INSERT DATA` query:
...
`DELETE/INSERT WHERE` queries allow you to delete and insert new quads, based on
quads that are already available:
...
By default, update queries will modify data within the given source. In some
cases, you may want to direct changes to another place. For example, if you have
multiple sources, but you want to direct all changes to a single source.
